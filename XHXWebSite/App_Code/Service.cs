...
public class Service : System.Web.Services.WebService
{
    ...
    public byte[] SearchAnswerDtl2Pic(string picName, string shopName, string subjectCode, string type, string code)
    {
        ...
        //{
            ...
            try
            {
                ...
                aliyun.GetObject("yrtech", "YIQIAODI" + @"/" + shopName + @"/" + subjectCode + @"/" + picName.Replace(".jpg", "") + ".jpg",
    ...
    public void DeletePicture(string shopName, string fileName, string subjectCode)
    {
        ...
        UploadFileToAliyun aliyun = new UploadFileToAliyun();
        aliyun.DeleteObject("yrtech", "YIQIAODI" + @"/" + shopName + @"/" + subjectCode + @"/" + fileName.Replace(".jpg", "") + ".jpg");
    }
    [WebMethod]
    ...
    #region 上传单店报告数据
    [WebMethod]
    public void UploadAllScore(string projectCode, string score, string mustLoss, string inUserId)
    {
        string sql = string.Format("exec [Upload_AllScoreInfo_S] '{0}','{1}','{2}','{3}'", projectCode, score, mustLoss, inUserId);
        CommonHandler.query(sql);
    }
    [WebMethod]
    ...
    [WebMethod]
    public void UploadAreaScore(string projectCode, string areaCode, string score, string mustLoss, string areaTypeCode, string inUserId)
    {
        string sql = string.Format("exec [Upload_AreaScoreInfo_S] '{0}','{1}','{2}','{3}','{4}','{5}'",
                                        projectCode, areaCode, score, mustLoss, areaTypeCode, inUserId);
        CommonHandler.query(sql);
    }
    [WebMethod]
    ...
    [WebMethod]
    public void UploadShopOrderNO(string projectCode, string shopCode, int orderNO_All, int orderNO_SmallNO)
    {
        string sql = string.Format("exec [Upload_ShopScoreInfo_UpdateOrderNO_S] '{0}','{1}','{2}','{3}'",
                                        projectCode, shopCode, orderNO_All, orderNO_SmallNO);
        CommonHandler.query(sql);
    }
    ...
    public DataSet[] GetShopReportDto(string projectCode, string shopCode)
    {
        ...
        ,SearchShopSubjectScoreInfo(projectCode,shopCode),SearchSaleScoreInfo_Week(projectCode,shopCode),SearSaleSubjectScore_Week(projectCode,shopCode)};
